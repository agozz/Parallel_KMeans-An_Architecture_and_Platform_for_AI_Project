...
{  
    ...
    {       
        ...
        {   
            ...
            #pragma omp parallel for reduction(+:clusters_points[m], sumX[m], sumY[m]) num_threads(num_thr) schedule(static, (int)v->size/num_thr)  
            for(int j=0; j<v->size; j++)
            {
                Point point = VectorPoint_get(v,j);
...
{
    FILE *fpt;
    fpt = fopen("k-means3_output.csv", "w+");
...
{   
    ...
    double start, stop;
    printf("\n");
    printf("%s\n\n", "Third solution for parallel K-Means");
    printf("%-50s %s\n", "", "Number of threads");
    printf("\n");
    printf("%-1s %s\n", "","Dataset size");
